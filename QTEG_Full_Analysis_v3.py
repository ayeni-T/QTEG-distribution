...
HOW TO RUN:
    ...
    3. Outputs created in the same folder:
       ...
       - fig2_simulation.png       (Simulation diagnostics with CIW)
       - fig3_simulation_cp.png    (Coverage probability)
       - fig4_bias.png             (Absolute bias)
       - fig5_ds1_bladder.png
       - fig7_ds3_boeing.png
       - fig3_all_datasets.png     (PDF / CDF / P-P for all 3 datasets)
       - fig4_hazard.png           (Standalone hazard functions, all 3 datasets)
================================================================================
"""
...
for alpha_true, beta_true, sc_label, _ in SCENARIOS:
    ...
    print(f"  {'n':>5}  {'Conv%':>6}  {'Bias(\u03b1)':>9}  {'RMSE(\u03b1)':>9}  "
          f"{'AvgSE(\u03b1)':>9}  {'CIW(\u03b1)':>9}  {'CP(\u03b1)%':>7}  "
          f"{'Bias(\u03b2)':>9}  {'RMSE(\u03b2)':>9}  "
          f"{'AvgSE(\u03b2)':>9}  {'CIW(\u03b2)':>9}  {'CP(\u03b2)%':>7}")
    print("  " + "-"*103)
    for n in SAMPLE_SIZES:
        ...
        ciw_alpha   = 2 * 1.96 * avgse_alpha          # avg 95% CI width
        ...
        ciw_beta    = 2 * 1.96 * avgse_beta            # avg 95% CI width
        cp_r    = 100.0 * cover_r / valid if valid > 0 else float('nan')

        print(f"  {n:>5}  {100*converged/NSIM:>6.1f}"
              f"  {bias_k:>+9.4f}  {rmse_alpha:>9.4f}  {avgse_alpha:>9.4f}  {ciw_alpha:>9.4f}  {cp_k:>7.1f}"
              f"  {bias_r:>+9.4f}  {rmse_beta:>9.4f}  {avgse_beta:>9.4f}  {ciw_beta:>9.4f}  {cp_r:>7.1f}")

        sim_rows.append(dict(scenario=sc_label, n=n,
                             conv=100*converged/NSIM,
                             bias_alpha=bias_k, rmse_alpha=rmse_alpha, avgse_alpha=avgse_alpha,
                             ciw_alpha=ciw_alpha, cp_alpha=cp_k,
                             bias_beta=bias_r,  rmse_beta=rmse_beta,  avgse_beta=avgse_beta,
                             ciw_beta=ciw_beta,  cp_beta=cp_r))
...
# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 3  DS1 + DS2 + DS3  --  PDF, CDF, P-P panels only  (3 rows x 3 cols)
# ─────────────────────────────────────────────────────────────────────────────
ds_triples = [
...
fig3.suptitle(
    'Figure 3: Empirical Applications -- Fitted PDF, CDF, and P-P Plots\n'
    'DS1 Bladder Cancer, DS2 Boeing 720 Failures, DS3 Malignant Melanoma',
...
for row, (ds_name, xlabel, ds_label) in enumerate(ds_triples):
    ...
    emp_cdf   = np.arange(1, n + 1) / n
    y_plot    = np.linspace(max(y.min() * 0.02, 0.005), y.max() * 1.02, 500)
    plot_comps = {m: fits[m] for m in ['Weibull','Gamma','LogNorm','Expon','Lindley'] if m in fits}
...
path3 = os.path.join(OUTPUT_DIR, 'fig3_all_datasets.png')
fig3.savefig(path3, dpi=180, bbox_inches='tight'); plt.close(fig3)
print(f"  Saved: {path3}")

# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 4  Standalone Hazard Functions  (3 rows x 1 col)
# Separated from Figure 3 to highlight the hazard-shape argument
# ─────────────────────────────────────────────────────────────────────────────
fig4, axes4 = plt.subplots(3, 1, figsize=(9, 14))
fig4.suptitle(
    'Figure 4: Fitted Hazard Functions -- DS1 Bladder Cancer,\n'
...
for row, (ds_name, xlabel, ds_label) in enumerate(ds_triples):
    ...
    for mname in HAZARD_MODELS:
        ...
        ax.plot(y_plot, h_c, color=col, lw=lw, ls=ls,
                label=model_label(mname, plot_comps[mname]))
    ax.set_xlabel(xlabel, fontsize=10); ax.set_ylabel('h(y)', fontsize=10)
    ax.set_title(f'{ds_label} Hazard Function', fontsize=10, fontweight='bold')
    ax.set_ylim(0, h_ceil)
    ax.legend(fontsize=7.5, framealpha=0.85); ax.grid(alpha=0.3)

fig4.tight_layout(rect=[0, 0, 1, 0.94])
path4 = os.path.join(OUTPUT_DIR, 'fig4_hazard.png')
fig4.savefig(path4, dpi=180, bbox_inches='tight'); plt.close(fig4)
print(f"  Saved: {path4}")
